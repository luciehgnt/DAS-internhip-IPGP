...
def make_dir(traces, pcc, parent_dir, folder, stats, ref_channel,i, time, window):
	...
	fname = f'{ref_channel}_{i}_{int(time*dt)}_{window}_{fdate}'
...
def cross_correlate(traces,parent_dir,window,lag,length,time,start_channel, end_channel, nb_channel,ref_channel,start_position,
	...
	if start_position == 'mid':
		...
		cc = np.zeros((2*int(window/dt),end_channel-start_channel+1))
	...
	if start_position == 'mid':
		ranges = np.arange(ref_channel-nb_channel,ref_channel+nb_channel+1,1)
	elif start_position == 'early':
		ranges = np.arange(ref_channel, ref_channel+nb_channel+1, 1)
	elif start_position == 'end':
		ranges = np.arange(ref_channel, ref_channel-nb_channel-1, -1)
	elif start_position == 'mid_all':
		ranges = np.arange(start_channel, end_channel+1, 1)
	...
	while time < length:
		if iteration%decimate == 0:
			...
			#for i in range(57,60): # use this when only interested in some channels
				...
				if onebit == True:
					x1 = one_bit(traces[time:time+int(window/dt),ref_channel])
					x2 = one_bit(traces[time:time+int(window/dt),i_channel])
				else:
					x1 = traces[time:time+int(window/dt),ref_channel]
				...
				if phase_cc == True:
					...
					try:
						...
						folder = f'Correlogram_{ref_channel}_{i_channel}_pcc/'
						make_dir(traces, pcc, parent_dir, folder, stats, ref_channel, i_channel, time, window)
					...
					try: # to account for cutted trace in last time window
						...
						cc[:,j] = cc[:,j] + corr  
						folder = f'Correlogram_{ref_channel}_{i_channel}_normal/'
						make_dir(traces, corr, parent_dir, folder, stats, ref_channel,i_channel, time, window)
					except:
						# print(len(corr),len(cc[:,j])) # show length
						print(f'Time window starting at {time*dt} s is shorter. Finishing the program ...')
						break
...
def auto_correlate(traces,parent_dir,window,lag,length,time,start_channel,end_channel,nb_channel,ref_channel,start_position,
			onebit,phase_cc,decimate=1,resample=1):
	...
	if start_position == 'mid':
		...
		# cc = np.zeros((2*int(window/dt),3249+1))
		cc = np.zeros((2*int(window/dt), end_channel-start_channel+1+1))
	else:
		cc = np.zeros((2*int(window/dt),nb_channel+1))
